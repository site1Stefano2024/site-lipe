...
//VALIDAR RADIO
jQuery.validar.isRadio = function($this) {
    if(lastItem !== $($this).attr('name')){
        lastItem = $($this).attr('name');
        ...
        .each(function() {
            ...
            if(req === 'radio'){
                if($this.parent().attr("label")){
                    label = jQuery.trim($this.parent().attr("label"));
                }else{
                    label = jQuery.trim($this.parent().parent().attr("label"));
                }
            }else if (typeof jQuery("label[for=" + $this.attr("id").replace("[]", "") + "]")[0] === "object") {
            ...
            try {
                if (r) {
                    ...
                    if(opc || req){
                        ...
                        if (req === "email") {
                            ...
                            if (!jQuery.validar.isRadio($this)) {
                                throw "O campo " + label + " deve ter uma opção selecionada.";
                            }
                        }else if (opc === "data"){
                            if(val !== ""){
                                if (!jQuery.validar.isData(val)) {
                ...
                if(req === 'radio'){
                    if($this.parent().attr("label")){
                        ...
                        $this.parent().parent().after(alerta);
                    }
                }else{
                    $this.after(alerta);
                }
